...
def generate_product_description(self, product_id: int):
    """AI 生成商品描述（异步）"""
    return asyncio.run(_generate_product_description(product_id))

@celery_app.task(bind=True, max_retries=3)
def batch_update_recommendations(self, user_id: int):
    """批量更新商品推荐（异步）"""
    return asyncio.run(_batch_update_recommendations(user_id))


@celery_app.task(bind=True, max_retries=3)
def generate_pricing_suggestion(self, product_id: int):
    """AI 动态定价建议（异步）"""
    return asyncio.run(_generate_pricing_suggestion(product_id))


@celery_app.task(bind=True, max_retries=3)
def generate_marketing_copy(self, product_id: int):
    """AI 个性化营销文案（异步）"""
    return asyncio.run(_generate_marketing_copy(product_id))
...
async def _generate_product_description(product_id: int) -> dict:
    async with AsyncSessionLocal() as db:
        ...
        fallback = (
            ...
            f"价格为 ¥{product.price}，适合希望快速完成智能购物决策的用户。"
        )

        description = fallback
        if settings.OPENAI_API_KEY:
            prompt = f"""为电商商品生成一段中文卖点描述，80字以内，语气专业可信。
商品名：{product.name}
品牌：{product.brand}
分类：{product.category}
价格：{product.price}
...
async def _batch_update_recommendations(user_id: int) -> dict:
    async with AsyncSessionLocal() as db:
        ...
        categories = [row[0] for row in bought.all() if row[0]]
...
async def _generate_marketing_copy(product_id: int) -> dict:
    async with AsyncSessionLocal() as db:
        ...
        fallback = (
            ...
            f"现在仅需 ¥{product.price}，适合立即加入购物清单。"
        )
        copy = fallback

        if settings.OPENAI_API_KEY:
            prompt = f"""为下面商品生成一段中文营销文案，60字以内，强调购买理由和行动号召。
商品名：{product.name}
品牌：{product.brand}
分类：{product.category}
价格：{product.price}
